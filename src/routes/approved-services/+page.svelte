<script lang="ts">
	...
	let groupedServices = $derived.by(() => {
		...
		return parents.map((parent: any) => {
			...
			const parentVersions = versions
				.filter((version: any) => version.parentId === parent._id)
				.map((version: any) => {
					// Check if this version is approved - get the most recent approval record
					const versionApprovals =
						approvals?.filter((approval: any) => approval.serviceVersionId === version._id) || [];

					// Sort by updatedAt descending to get the most recent
					const mostRecentApproval = versionApprovals.sort(
	...
	async function toggleServiceApproval(serviceVersionId: string, serviceName: string) {
		...
		try {
			const result = await client.mutation(
				(api as any).serviceVersions.toggleOrganizationServiceApproval,
				{
...
<div class="container mx-auto p-8">
	<div class="mb-8">
		...
		<p class="text-gray-600">
			Manage service approvals for the selected organization. Toggle services on/off to test
			different scenarios.
		</p>
	</div>
	...
	{:else if !serviceParents?.data || !serviceVersions?.data}
		<div class="text-center py-12">
			...
			<p class="text-gray-500 mb-4">
				No service parents or versions found. Please seed the data first.
			</p>
			<a
					...
					<div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
						<div class="flex items-center justify-between">
							...
							<div class="flex items-center space-x-2">
								...
								>
									{serviceParent.versions.length} version{serviceParent.versions.length !== 1
										? 's'
										: ''}
								</span>
										...
										<div class="flex items-center space-x-4 mt-2 text-xs text-gray-500">
											...
											{#if version.deprecatedAt}
												<span class="text-red-600"
													>Deprecated: {new Date(version.deprecatedAt).toLocaleDateString()}</span
												>
											{/if}
										</div>
									</div>
