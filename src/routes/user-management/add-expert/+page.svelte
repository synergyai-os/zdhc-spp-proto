<script lang="ts">
	...
	const isLoadingServices = $derived(
		serviceVersions?.isLoading || organizationApprovals?.isLoading || false
	);
	const hasError = $derived(
		existingUsers?.error || serviceVersions?.error || organizationApprovals?.error || false
	);
	...
	const canProceedFromStep4 = $derived(
		experience.length > 0 && experience.every((exp) => exp.title.trim() !== '')
	);
	const canProceedFromStep5 = $derived(
		education.length > 0 && education.every((edu) => edu.school.trim() !== '')
	);
		...
		try {
			...
			if (isDraftMode) {
				...
				const existingUser = existingUsers?.data?.find(
					(user) => user.email.toLowerCase() === pdcUserData!.email.toLowerCase()
				);
	...
	function handleCancel() {
		if (userId) {
			...
			if (
				confirm(
					'Your progress has been saved. You can continue editing this expert profile at any time from the User Management page. Are you sure you want to leave?'
				)
			) {
				window.history.back();
			}
				...
				<div class="flex justify-between mb-4">
					<div class="flex-1 text-center">
						<div class="flex flex-col items-center">
							<div
								class={`w-8 h-8 rounded-full flex items-center justify-center font-bold mb-2 text-xs ${
		...
		<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
			...
			{#if currentStep === 1}
				<div>
					...
					<p class="text-gray-600 mb-6">
						Enter the expert's email to check if they exist in PDC (Platform for Data Collection)
					</p>
					...
					{#if isLoadingUsers}
						<div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
							<div class="flex items-center">
								<div
									class="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-500 mr-3"
								></div>
						...
						<div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
							<div class="flex items-center">
								...
								<span class="text-red-700"
									>Error loading user database. Please refresh the page.</span
								>
							</div>
						</div>
					{/if}
					...
					<div class="space-y-4">
								...
								<div class="flex items-start">
									...
									<div>
										...
										<p class="text-sm text-green-700 mt-1">
											Click "Next" to review their information
										</p>
									</div>
								</div>
							</div>
						{:else if userExists === false}
							<div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
								<div class="flex items-start">
									<svg
										class="w-5 h-5 text-yellow-500 mt-0.5 mr-3"
										fill="currentColor"
										viewBox="0 0 20 20"
									>
										<path
									...
									<div class="flex-1">
										...
										<p class="text-sm text-yellow-700 mt-1">
											This user doesn't have a PDC account yet. You can invite them to join ZDHC.
										</p>
				...
				<div>
					{#if isDraftMode}
						<h2 class="text-xl font-bold text-gray-800 mb-4">
							Step 2: Draft Mode - Invitation Sent
						</h2>
						<p class="text-gray-600 mb-6">
							You're adding an expert in draft mode. The user has been invited to join ZDHC and will
							complete their profile information when they accept the invitation.
						</p>

						<div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
							<div class="flex items-start">
								<svg
									class="w-6 h-6 text-blue-500 mt-0.5 mr-4"
									fill="currentColor"
									viewBox="0 0 20 20"
								>
									<path
								...
								<div class="flex-1">
									...
									<div class="space-y-2 text-sm text-blue-700">
										...
										<p>
											<strong>Mode:</strong> Draft - Expert assignments will be created and linked when
					...
					{:else}
						...
						<p class="text-gray-600 mb-6">
							Review the user's default data from PDC. This data is owned by the user and cannot be
							edited.
						</p>
							...
							<div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
								...
								<div class="mt-6 pt-4 border-t border-blue-300">
									<div class="flex items-center text-sm text-blue-700">
										...
										<span
											><strong>Note:</strong> This data is managed by the user in PDC and cannot be modified
											here.</span
										>
									</div>
								</div>
				...
				<div>
					...
					{#if isDraftMode}
						<p class="text-gray-600 mb-2">
							Choose which services this expert will provide and whether they are a LEAD expert for
							each service. These assignments will be created in draft mode and linked when the user
					...
					{#if currentOrgId}
						<div class="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
							<div class="flex items-center text-sm text-green-700">
								...
								<span
									><strong>Approved Services:</strong> Showing only services that your organization
									is approved to provide ({availableServices.length} available)</span
								>
							</div>
						</div>
					{:else}
						<div class="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
							<div class="flex items-center text-sm text-yellow-700">
								...
								<span
									><strong>No Organization Selected:</strong> Please select an organization to see approved
									services</span
								>
							</div>
						</div>
						...
						<div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
							<div class="flex items-center">
								...
								<span class="text-yellow-700"
									>No services available. Please seed the database first.</span
								>
							</div>
						</div>
					{/if}
								...
								>
									...
									{#if selectedServices.includes(service)}
										<button
											...
											class="ml-3 px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 cursor-pointer transform hover:scale-105 hover:shadow-md {serviceRoles[
												service
											] === 'lead'
										...
										>
											<span class="flex items-center gap-1">
												<svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
													<path
														fill-rule="evenodd"
														d="M3 3a1 1 0 000 2v11a1 1 0 102 0V5a1 1 0 011-1h7a1 1 0 100 2H6v10a1 1 0 102 0V6h7a1 1 0 100-2H6a3 3 0 00-3 3z"
														clip-rule="evenodd"
													/>
												</svg>
							...
							<div class="space-y-2">
								{#each selectedServices as service}
									<div class="flex items-center justify-between text-sm">
										...
										<span
											class="px-2 py-1 rounded text-xs font-medium {serviceRoles[service] === 'lead'
			...
			{:else if currentStep === 4}
				<div>
					...
					<p class="text-gray-600 mb-2">
						Add the expert's professional experience. This information is managed by SPP and
					...
					<div class="mb-6 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
						<div class="flex items-center text-sm text-yellow-700">
							...
							<span
								><strong>Important:</strong> SPP owns this data and is legally responsible for employee
								information</span
							>
						</div>
					</div>
						...
						{#each experience as exp, index}
							<div class="border border-gray-200 rounded-lg p-4">
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
									<div>
										<label
											for="exp-title-{exp.id}"
											class="block text-sm font-medium text-gray-700 mb-2">Job Title *</label
										>
										<input
											id="exp-title-{exp.id}"
											type="text"
									...
									<div>
										<fieldset class="block">
											...
											<div class="flex items-center">
												...
												/>
												<label for="exp-current-{exp.id}" class="ml-2 text-sm text-gray-700"
			...
			{:else if currentStep === 5}
				<div>
					...
					{#if isDraftMode}
						<p class="text-gray-600 mb-2">
							Add the expert's educational background. This information will be stored in draft mode
							and linked when the user accepts the invitation.
						</p>
					{:else}
						<p class="text-gray-600 mb-2">
							Add the expert's educational background. This information is managed by SPP and
							required for legal compliance.
						</p>
					{/if}
					<div class="mb-6 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
						...
						{#each education as edu, index}
							<div class="border border-gray-200 rounded-lg p-4">
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									<div>
										<label
											for="edu-degree-{edu.id}"
											class="block text-sm font-medium text-gray-700 mb-2">Degree</label
										>
										<input
									...
									<div>
										<label
											for="edu-field-{edu.id}"
											class="block text-sm font-medium text-gray-700 mb-2">Field of Study</label
										>
										<input
											id="edu-field-{edu.id}"
											type="text"
									...
									<div>
										<label
											for="edu-year-{edu.id}"
											class="block text-sm font-medium text-gray-700 mb-2">Graduation Year</label
										>
										<input
											id="edu-year-{edu.id}"
											type="number"
