<script lang="ts">
	...
	import { saveExpertProfileWithServices } from '$lib/services/expertService';
	import { api } from '$lib';
	...
	async function handleSave(queryData: any) {
		...
		console.log('🔍 Calling saveExpertProfileWithServices with:', {
			cvId: queryData.currentCVData._id,
		...
		const result = await saveExpertProfileWithServices(client, {
			cvId: queryData.currentCVData._id as any,
			experience: queryData.experience,
			education: queryData.education,
			userSelectedServices: expertEditState.userSelectedServices,
			userServiceRoles: expertEditState.userServiceRoles,
			existingServiceAssignments: queryData.existingServiceAssignments?.data || [],
			serviceVersionsData: queryData.serviceVersionsData || [],
	...
	{:else if expertId}
		...
		<ExpertQueries expertId={expertId} orgId={expertEditState.validOrgId}>
			{#snippet children(queryDataFromComponent: any)}
				<!-- Initialize store state with query data when available -->
				{@const _ = (() => {
					if (queryDataFromComponent?.selectedServices && queryDataFromComponent?.serviceRoles) {
						expertEditStore.initializeUserState(
							queryDataFromComponent.selectedServices,
							queryDataFromComponent.serviceRoles,
							queryDataFromComponent.experience || [],
							queryDataFromComponent.education || []
						);
					}
				})()}
		...
		{:else}
			<!-- Expert Info Header -->
						...
						<ExpertEditSteps 
							...
							bind:selectedServices={expertEditState.userSelectedServices}
							bind:serviceRoles={expertEditState.userServiceRoles}
