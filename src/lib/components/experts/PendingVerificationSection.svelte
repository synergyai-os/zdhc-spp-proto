<script lang="ts">
	...
	let { experts, isLoading = false, onSendReminder, onViewDetails, onCompleteProfile }: Props = $props();
	...
	<div class="bg-white border border-amber-200 rounded-lg shadow-sm overflow-hidden mb-6">
							...
							<td class="px-6 py-4 whitespace-nowrap">
								<div class="flex items-center">
									<div class="flex-shrink-0 h-10 w-10">
										<div class="h-10 w-10 rounded-full bg-amber-500 text-white flex items-center justify-center font-semibold text-sm">
											{expert.avatar}
										</div>
									</div>
									<div class="ml-4">
		...
		<div class="px-6 py-3 bg-amber-50 border-t border-amber-200">
			<div class="flex items-center justify-between text-sm text-amber-800">
				<span>Showing {experts.length} expert{experts.length !== 1 ? 's' : ''} pending verification</span>
