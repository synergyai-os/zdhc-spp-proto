<script lang="ts">
	import { organizationStore, currentOrganizationName, organizationOptions } from '$lib/stores/organization.svelte';
				...
				{:else}
					{#each availableOrgs as org (org.id)}
						...
						<button
							...
							class="w-full px-4 py-3 text-left hover:bg-gray-50 focus:bg-gray-50 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed {isCurrentOrg ? 'bg-blue-50 border-l-4 border-blue-500' : ''}"
						>
							<div class="flex items-center justify-between">
								<div class="flex-1 min-w-0">
									<div class="flex items-center space-x-2">
										<!-- Organization Type Badge -->
										<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {org.type === 'solution_provider' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}">
