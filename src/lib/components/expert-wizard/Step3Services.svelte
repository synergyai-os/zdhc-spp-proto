	...
	{:else}
		<div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
			<div class="flex items-center text-sm text-blue-700">
				...
				<span><strong>Prototype Mode:</strong> Showing all available services ({availableServices.length} available)</span>
			</div>
		</div>
	{/if}
			...
			<div class="flex items-center justify-between p-4 border-2 rounded-lg transition-all {
				...
				{#if selectedServices.includes(service.name)}
					<button
						...
						class="ml-3 inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-medium transition-colors {
							serviceRoles[service.name] === 'lead'
