...
<div
	class="border border-gray-200 rounded-lg p-4 transition-all duration-200 {!canSelect
		...
		<div class="flex-1 min-w-0">
			<div class="flex items-start justify-between">
				<div class="flex-1">
					...
					<div class="mb-2">
						<h3 class="text-lg font-semibold text-gray-900 {!canSelect ? 'text-gray-500' : ''}">
							{expert.userName}
						</h3>
					...
					<div class="mb-3">
						<div class="flex items-center space-x-2 mb-1">
							...
							<span
								class="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded font-medium {!canSelect
					...
					<div class="mb-3">
						<span
							class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {expert.role ===
							'lead'
								? 'bg-yellow-100 text-yellow-800'
								: 'bg-gray-100 text-gray-800'} {!canSelect ? 'opacity-50' : ''}"
						>
							{#if expert.role === 'lead'}
					...
					{#if !canSelect}
						...
						>
							...
							<span class="text-sm font-medium text-yellow-800"> Expert account not verified </span>
						</div>
					{/if}
				</div>
				...
				<div class="flex-shrink-0 text-right">
					<div class="text-lg font-bold text-gray-900 {!canSelect ? 'text-gray-500' : ''}">
						€{expert.price.toFixed(2)}
					</div>
					<div class="text-xs text-gray-500 {!canSelect ? 'text-gray-400' : ''}">per service</div>
				</div>
			</div>
		</div>
	</div>
