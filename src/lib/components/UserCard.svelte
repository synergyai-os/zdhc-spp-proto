...
<div
	class="border border-gray-200 rounded-lg {containerClasses[
		size
	]} {leadBackground} hover:shadow-md transition-shadow"
>
	<div class="flex items-center {size === 'sm' ? 'space-x-2' : 'space-x-3'}">
		<div
			class="{sizeClasses[size]} {user.isLead
				? 'bg-yellow-100'
		...
		<div class="flex-1">
			...
			{#if user.additionalServices && user.additionalServices.length > 0}
				<span
					class="inline-flex px-1 py-0.5 text-xs font-semibold rounded-full bg-blue-100 text-blue-800"
				>
		...
		{#if showBadge}
			<div class="flex flex-col items-end space-y-1">
				{#if user.isLead}
					<span
						class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-yellow-200 text-yellow-800"
					>
						LEAD
					</span>
				{/if}
				...
				{#if user.status && user.status !== 'active'}
					<span
						class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {statusClasses[
							user.status
						]}"
					>
						{user.status}
				...
				{#if user.badge && user.badgeColor}
					<span
						class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {badgeClasses[
							user.badgeColor
						]}"
					>
						{user.badge}
