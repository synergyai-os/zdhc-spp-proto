...
function createOrganizationStore() {
  ...
  return {
    ...
    isManagerOrAdmin: (): boolean => {
      ...
      const unsubscribe = subscribe(state => {
        isManagerOrAdmin = state.currentStaffMember?.role === 'admin' || 
                          state.currentStaffMember?.role === 'manager' || false;
      });
      unsubscribe(); // Immediately unsubscribe after getting the value
...
export const organizationOptions = derived(
  organizationStore,
  ($store) => $store.availableOrganizations.map(org => ({
    id: org._id,
    name: org.name,
    type: org.type,
    status: org.status,
...
export const organizationContextSummary = derived(
  organizationStore,
  ($store) => ({
