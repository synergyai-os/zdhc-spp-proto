...
export const getCurrentUserSession = query({
	...
	handler: async (ctx, args) => {
		...
		return await ctx.db
			.query('userSessions')
			.filter((q) => q.and(q.eq(q.field('userId'), args.userId), q.gt(q.field('expiresAt'), now)))
			.order('desc')
			.first();
	}
});
...
export const createShellAssignment = mutation({
	...
	handler: async (ctx, args) => {
		...
		// Create a true shell assignment with NO service version assigned
		// This will be updated in step 3 when actual services are selected
		return await ctx.db.insert('expertAssignments', {
			userId: args.userId,
			organizationId: args.organizationId,
			serviceVersionId: undefined, // No service assigned - this is a true shell assignment
			role: undefined, // No role assigned until service is selected
			experience: [],
			education: [],
...
export const getExpertAssignmentsByUserId = query({
	args: { userId: v.id('users') },
	handler: async (ctx, args) => {
		const assignments = await ctx.db
			.query('expertAssignments')
		...
		// Enrich with service version details
		const enrichedAssignments = await Promise.all(
			assignments.map(async (assignment) => {
				// Handle shell assignments where serviceVersionId might be undefined
				const serviceVersion = assignment.serviceVersionId ? await ctx.db.get(assignment.serviceVersionId) : null;
...
export const updateMultipleExpertStatuses = mutation({
	...
	handler: async (ctx, args) => {
		...
		const results = await Promise.all(args.assignmentIds.map((id) => ctx.db.patch(id, updateData)));
...
export const seedInitialData = mutation({
	...
	handler: async (ctx) => {
		...
		return {
			...
			expertAssignments: [],
			message:
				'Initial data seeded. Run seedOrganizationServiceApprovals to approve services for organizations.'
		};
	}
});
